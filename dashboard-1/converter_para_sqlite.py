...
def main():
    ...
    arquivo_nps             = diretorio / "DBV Capital_NPS.csv"
    ...
    saida_nps             = diretorio / "DBV Capital_NPS.db"
    ...
    # Processar arquivo NPS
    if arquivo_nps.exists():
        print(f"\nProcessando arquivo NPS: {arquivo_nps.name}")
        importar_nps_para_sqlite(arquivo_nps, saida_nps)
    else:
        print(f"\nAviso: Arquivo {arquivo_nps.name} não encontrado.")

    print("\nProcesso concluído!")

def criar_tabela_nps(conn):
    cursor = conn.cursor()
    cursor.execute('''
    ...
    ''')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_nps_codigo_assessor ON nps(codigo_assessor)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_nps_data_resposta ON nps(data_resposta)')
    conn.commit()

def importar_nps_para_sqlite(caminho_arquivo, caminho_saida='dashboard.db'):
    """
    Importa o arquivo CSV do NPS para o banco de dados SQLite.
    
    Args:
        caminho_arquivo: Caminho para o arquivo CSV do NPS
        caminho_saida: Caminho para o arquivo SQLite de saída
    """
    try:
        ...
        df = df.rename(columns={
            ...
            'XP - Relacionamento - Aniversário - NPS Assessor': 'tipo_pesquisa',
            'Status': 'status',
            'Código Assessor': 'codigo_assessor',
        ...
        colunas = [
            'survey_id', 'usuario_id', 'customer_id', 'data_resposta',
            'pesquisa_relacionamento', 'tipo_pesquisa', 'status',
            'codigo_assessor', 'notificacao', 'nota', 'comentario'
        ]
        ...
        print(f"Inserindo {len(df)} registros na tabela NPS...")
        df.to_sql('nps', conn, if_exists='replace', index=False)
        
        print("Importação concluída com sucesso!")
        return True
        
    except Exception as e:
        print(f"Erro ao importar dados do NPS: {str(e)}")
        return False
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    # Adiciona a opção de importar NPS
    import argparse
    
    parser = argparse.ArgumentParser(description='Importar dados para SQLite')
    parser.add_argument('--nps', type=str, help='Caminho para o arquivo CSV do NPS')
    parser.add_argument('--output', type=str, default='dashboard.db', help='Caminho para o arquivo SQLite de saída')
    
    args = parser.parse_args()
    
    if args.nps:
        importar_nps_para_sqlite(args.nps, args.output)
    else:
        main()
